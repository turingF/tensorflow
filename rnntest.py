...
# show data
steps = np.linspace(0, 500, 397, dtype=np.float32)
from file_open import failnum,failtime
x_np = failnum; y_np = failtime    # float32 for converting torch FloatTensor
...
for step in range(60):
    ...
    start, end = step , (step+1) # time range


    # use sin predicts cos
    steps = np.linspace(start, end, TIME_STEP)
    x = np.array(failnum)[np.newaxis, :, np.newaxis]    # shape (batch, time_step, input_size)
    y = np.array(failtime)[np.newaxis, :, np.newaxis]
    ...
    plt.ylim((-1000, 1000)); plt.draw(); plt.pause(0.05)
